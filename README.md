...
- **Widgets** — `Button` (variants, icons, ripple, press-scale), `ToggleSwitch`, `Checkbox` (draw-on check), `Slider`, `TextField` (selection, clipboard, word-jump, placeholder), `Dropdown` (animated popup layer), `ScrollPanel` (smooth scrolling, fading scrollbar), `Label`, `ProgressBar` (+ indeterminate), `Spinner`, `ShaderView`, `Panel` cards with shadows or frosted glass.
